...
from gpustack.utils.auto_load import llama_cpp_slot_capacity
...
class AutoScalingTask:
    ...
    async def _evaluate(self, session, model: Model):
        ...
        if model.ready_replicas < model.replicas:
            ...
            await self._clear_pressure(session, model)
            return
        ...
        if snap.waiting_total > 0 and model.replicas < ceiling:
            ...
            if state.up_first_met_at is None:
                ...
                await self._set_pressure(session, model, now)
                return
        ...
        if model.replicas > 1 and snap.waiting_total == 0:
            if snap.kv_supported:
                ...
                slot_capacity = llama_cpp_slot_capacity(model)
    ...
    async def _set_pressure(self, session, model: Model, now: datetime):
        if model.scale_pressure_since is None:
            model.scale_pressure_since = now
            await model.update(session)

    async def _clear_pressure(self, session, model: Model):
        if model.scale_pressure_since is not None:
            model.scale_pressure_since = None
            await model.update(session)
